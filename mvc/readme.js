 ...
 *
 * 8. MVC
 *  MVC不是设计模式 -> 架构 -> 项目如何设计分层 -> 设计思想/方案
 *  V: View -> 视图层 -> 封装HTML并且提供动态嵌入数据的功能
 *  C: Controller -> 控制器层 -> (123 / 13)
 *      1. 调用模型层对数据库的操作,并获取响应数据
 *      2. 将数据传递给模板引擎,并完整返回HTML
 *      3. 将数据直接响应给客户端
 *      4. 将HTML响应给客户端
 *  M: Model -> 模型层 -> 数据模型的操作、数据模型的定义
 *      1. 根据控制器层的需求,对数据库内的数据进行增删改查的操作
 *      2. 返回控制器层需要的数据
 *
 * 9. Node中间层
 *  客户端URL -> Node -> controller -> Java API -> data -> controller -> data -> ejs -> html -> controller -> 客户端
 *  客户端 <--> Node <--> 数据端
 *             View
 *              vc        m
 */
